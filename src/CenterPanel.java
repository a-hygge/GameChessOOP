        ...
        if (thisPiece.color == PieceColor.WHITE) {
            ...
            for (int dir = 0; dir < 4; dir++) {
                for (int i = 1; i < maxMove; i++) {
                    int newX = x + dx[dir] * i;
                    int newY = y + dy[dir] * i;
                    if (!CheckValidCoordinate(newX, newY)) break;
                    ChessPiece _chessPiece = boardCells[newX][newY].currentChessPiece;
                    if(_chessPiece != null){
                        if(_chessPiece.color != thisPiece.color){
                            boardCells[newX][newY].SetColor(false);
                        }
                        break;
                    }
                    else{
                        boardCells[newX][newY].SetColor(true);
                    }
                }
            }
